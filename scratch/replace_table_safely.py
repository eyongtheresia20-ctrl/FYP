with open(r'lib/views/dashboards/admin_dashboard.dart', 'r', encoding='utf-8', errors='ignore') as f:
    text = f.read()

idx_schools = text.find("Registered Schools Directory")
idx_else_schools = text.find("] else ...[", idx_schools)
print("idx_else_schools:", idx_else_schools)

# Find the end of this Column
# It ends right before `                                           ],` of the container
idx_container_close = text.find("                                          ],", idx_else_schools)
print("idx_container_close:", idx_container_close)
...
if idx_else_schools != -1 and idx_container_close != -1:
    text = text[:idx_else_schools] + new_content + text[idx_container_close:]
    print("SUCCESS: REPLACED TABLE UI SAFELY")
    with open(r'lib/views/dashboards/admin_dashboard.dart', 'w', encoding='utf-8') as f:
        f.write(text)
else:
    print("FAILED TO LOCATE BOUNDARIES")
