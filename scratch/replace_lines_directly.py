...
# Find start line: "// Full-Width Structured Table Header"
start_idx = -1
for i, l in enumerate(lines):
    if "// Full-Width Structured Table Header" in l:
        start_idx = i
        break

# Find end line: "}).toList()," right before line 2640
end_idx = -1
for i in range(start_idx, len(lines)):
    if "}).toList()," in lines[i] and "                                               )," in lines[i+1]:
        end_idx = i + 2
        break
...
if start_idx != -1 and end_idx != -1:
    new_lines = lines[:start_idx] + new_table_lines + lines[end_idx:]
    with open(r'lib/views/dashboards/admin_dashboard.dart', 'w', encoding='utf-8') as f:
        f.writelines(new_lines)
    print("SUCCESS: REPLACED LINES DIRECTLY")
else:
