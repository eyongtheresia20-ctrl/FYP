...
admin_php = admin_php.replace(
    "SELECT id, name, region, division, town FROM schools ORDER BY region, division, name",
    "SELECT id, name, region, division, town, COALESCE(is_active, 1) AS is_active FROM schools ORDER BY region, division, name"
)
...
# 2. Update lib/views/dashboards/admin_dashboard.dart
with open(r'lib/views/dashboards/admin_dashboard.dart', 'r', encoding='utf-8') as f:
    dart_code = f.read()

# Add edit and delete methods for school in admin_dashboard.dart
school_methods = """  void _showEditSchoolDialog(Map<String, dynamic> school) {
...
# Insert school_methods right above _showAddSchoolDialog
if "void _showEditSchoolDialog" not in dart_code:
    dart_code = dart_code.replace("  void _showAddSchoolDialog() {", school_methods + "\n  void _showAddSchoolDialog() {")

# Replace Table UI in admin_dashboard.dart
old_table_ui_start = "                                               // Full-Width Structured Table Header"
old_table_ui_end = "                                                 }).toList(),\n                                               ),\n                                             ],"

new_table_ui = """                                               // Full-Width Structured Table Header
                                               Container(
                                                 width: double.infinity,
...
idx_t_start = dart_code.find(old_table_ui_start)
idx_t_end = dart_code.find(old_table_ui_end) + len(old_table_ui_end)

if idx_t_start != -1 and idx_t_end != -1:
    dart_code = dart_code[:idx_t_start] + new_table_ui + dart_code[idx_t_end:]
    print("SUCCESS: REPLACED TABLE UI IN ADMIN DASHBOARD")
else:
    print(f"FAILED TO FIND TABLE UI: idx_start={idx_t_start}, idx_end={idx_t_end}")

with open(r'lib/views/dashboards/admin_dashboard.dart', 'w', encoding='utf-8') as f:
    f.write(dart_code)

print("SUCCESS: IMPLEMENTED FULL SCHOOL CRUD & ACTIONS")
