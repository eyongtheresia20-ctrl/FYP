...
class DelegateDashboard extends StatelessWidget {
  ...
  final bool isEn;
  const DelegateDashboard({super.key, required this.user, required this.isDarkMode, required this.isEn});
  ...
  Widget build(BuildContext context) {
    final title = user.isRegionalDelegate
        ? (isEn ? 'Regional Delegate' : 'Délégué Régional')
        : (isEn ? 'Divisional Delegate' : 'Délégué Départemental');
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Icon(Icons.location_city_rounded, color: _accent, size: 22),
                      const SizedBox(width: 10),
                      Text(isEn ? 'Territorial Overview' : 'Aperçu Territorial', style: TextStyle(color: _text, fontWeight: FontWeight.w800, fontSize: 16)),
                    ],
                  ),
                  ...
                  Center(
                    child: Column(
                      children: [
                        Icon(Icons.map_outlined, color: _sub, size: 40),
                        const SizedBox(height: 10),
                        Text(
                          isEn ? 'School & division reports will load here.' : 'Les rapports par établissement et département s\'afficheront ici.',
