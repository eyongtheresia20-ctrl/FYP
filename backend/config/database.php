        ...
        if (!$connected) {
            try {
                ...
                if (function_exists('respondError')) {
                    respondError('Database connection failed. Please start MySQL in XAMPP Control Panel.', 500);
                } else {
                    http_response_code(500);
                    echo json_encode(['success' => false, 'message' => 'Database connection failed. Please start MySQL in XAMPP Control Panel.']);
                    exit();
                }
            }
        }
